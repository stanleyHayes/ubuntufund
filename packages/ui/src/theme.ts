...
export const NEUMORPHIC_WHITE_VARS = {
  ...
  '--neu-inset':
    'inset 3px 3px 7px rgba(38,55,44,0.11), inset -3px -3px 7px rgba(255,255,255,0.45)',
} as const
...
export const THEME_SKINS: { id: ThemeSkin; label: string; blurb: string }[] = [
  {
    id: 'neumorphism',
    label: 'Neumorphism',
    blurb: 'Soft, embossed surfaces — the Ujimora default.',
  },
  {
    id: 'claymorphism',
    label: 'Claymorphism',
    blurb: 'Puffy, playful clay with deep soft shadows.',
  },
  {
    id: 'glassmorphism',
    label: 'Glassmorphism',
    blurb: 'Frosted, translucent panels with a subtle blur.',
  },
...
function getSurfaceVars(skin: ThemeSkin, dark: boolean): Record<string, string> {
  ...
  if (skin === 'glassmorphism') {
    ...
    return {
      ...
      '--neu-raised-hover': dark
        ? '0 12px 40px rgba(0,0,0,0.5)'
        : '0 12px 40px rgba(31,38,28,0.22)',
...
export function getSkinVars(skin: ThemeSkin, dark: boolean): Record<string, string> {
  ...
  return {
    ...
    ...Object.fromEntries(
      ['raised', 'raised-hover', 'subtle', 'inset'].map((key) => [
        `--forest-${key}`,
        forest[`--neu-${key}`],
      ]),
    ),
    ...
    components: {
      MuiButton: {
        styleOverrides: {
          ...
          text: {
            ...
            '&:active': { boxShadow: 'var(--neu-inset)' },
          },
        },
        defaultProps: {
      ...
      MuiCssBaseline: {
        styleOverrides: {
          ...
          '.MuiBox-root, .MuiPaper-root, .MuiCard-root, .MuiDivider-root, .MuiButton-root, .MuiIconButton-root, .MuiToggleButton-root, .MuiChip-root, .MuiOutlinedInput-notchedOutline, .MuiTableCell-root':
            {
