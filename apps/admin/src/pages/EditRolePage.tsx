...
// Seed mock roles with IDs
const SEEDED_ROLES = DEFAULT_ROLES.map((r, i) => ({
  ...r,
  id: `role_seed_${i}`,
  createdAt: new Date(),
  updatedAt: new Date(),
...
export default function EditRolePage() {
  ...
  const role = useMemo(() => SEEDED_ROLES.find((r) => r.id === id), [id])

  const [formName, setFormName] = useState(role?.name ?? '')
  const [formDescription, setFormDescription] = useState(role?.description ?? '')
  const [formPerms, setFormPerms] = useState<Set<PermissionString>>(
  ...
  if (!role) {
    return (
      <Box sx={{ p: 3, maxWidth: 1400, mx: 'auto' }}>
        <ItemNotFound itemType="Role" onBack={() => navigate('/roles')} backLabel="Back to Roles" />
      </Box>
    )
  }
  ...
  function handleSave() {
    ...
    setSnackbar({ open: true, message: `Role "${formName.trim()}" updated.`, severity: 'success' })
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1400, mx: 'auto', animation: `${fadeSlide} 0.4s ease both` }}>
      ...
      <Breadcrumbs separator={<NavigateNextIcon fontSize="small" />} sx={{ mb: 3 }}>
        ...
        <Typography sx={{ fontSize: '0.85rem', color: 'text.primary' }}>Edit Role</Typography>
      </Breadcrumbs>
