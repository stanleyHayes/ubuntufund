...
export default function EditPlanPage() {
  ...
  // Load original plan from static data (in production would fetch from API)
  const originalPlan = useMemo<PlanWithMeta>(() => {
    const found = Object.values(SUBSCRIPTION_PLANS).find((p) => p.tier === tier)
    if (!found) {
      return {
    ...
    return { ...found, isActive: true, sortOrder: 0, audience: 'both' as PlanAudience }
  }, [tier])

  const [plan, setPlan] = useState<PlanWithMeta>({ ...originalPlan })
  ...
  // Detect whether any field differs from the original
  const hasChanges = useMemo(() => {
    const keys = Object.keys(originalPlan) as (keyof PlanWithMeta)[]
    return keys.some((k) => plan[k] !== originalPlan[k])
  }, [plan, originalPlan])
  ...
  function handleSave() {
    ...
    // In production this would PUT/PATCH to the API
    setSnack({ open: true, message: `Plan "${plan.name}" updated`, severity: 'success' })
              ...
              <Grid container spacing={2}>
                <Grid size={{ xs: 12, sm: 6 }}>
                  <TextField
                    ...
                    size="small"
                    disabled
                    helperText="Cannot change tier key"
                  />
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }}>
