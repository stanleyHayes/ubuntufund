...
interface Summary { scanned: number; settled: number; detected: number; failed: number; pending: number; errored: number; blocked?: number; issues?: Array<{ donationIntentId: string; provider: string; reason: 'missing_reference' | 'provider_unavailable' }> }

const issueLabel = (reason: string) => reason === 'missing_reference' ? 'Missing provider reference' : 'Provider unavailable'
    ...
    <Box component="section" aria-label="Crypto contributions" sx={{ mb: 4, border: '1px solid', borderColor: 'divider', borderRadius: 4, overflow: 'hidden', bgcolor: 'background.paper' }}>
      ...
      <Box sx={{ p: { xs: 2.5, md: 3 }, display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 1fr' }, gap: 4 }}>
        ...
        <Box>
          ...
          {summary && <Box role="status" sx={{ mt: 2 }}><Typography sx={{ fontWeight: 750, mb: 1 }}>Latest check results</Typography><Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 1 }}>{Object.entries(summary).filter(([, value]) => typeof value === 'number').map(([key, count]) => <Box key={key} sx={{ p: 1.5, bgcolor: 'action.hover', borderRadius: 2 }}><Typography sx={{ fontSize: '1.5rem', fontWeight: 800 }}>{count}</Typography><Typography sx={{ fontSize: '.75rem', textTransform: 'capitalize' }}>{key === 'errored' ? 'Check errors' : key}</Typography></Box>)}</Box>{!!summary.issues?.length && <Alert severity="warning" sx={{ mt: 1 }}>These deposits need investigation before automated recovery can continue. Verify the original provider configuration and records; do not create replacement deposits or manually mark them paid.{summary.issues.map(issue => <Box key={issue.donationIntentId} sx={{ mt: 1, overflowWrap: 'anywhere' }}><Typography variant="body2" sx={{ fontWeight: 700 }}>{issue.donationIntentId}</Typography><Typography variant="body2">{issue.provider} · {issueLabel(issue.reason)}</Typography></Box>)}</Alert>}{summary.errored > 0 && <Alert severity="warning" sx={{ mt: 1 }}>Some provider checks failed. Review provider logs before retrying.</Alert>}</Box>}
        </Box>
      </Box>
