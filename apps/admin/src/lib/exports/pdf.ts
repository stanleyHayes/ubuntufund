...
export function pdfDefinition(report: ExportReport): TDocumentDefinitions {
  ...
  const content: Content[] = [
    { text: report.title, fontSize: 24, bold: true, color: REPORT_COLORS.forest, margin: [0, 4, 0, 8] },
  ...
  return {
    ...
    defaultStyle: { font: 'Outfit', fontSize: 10, color: REPORT_COLORS.ink, lineHeight: 1.15 },
    styles: { section: { fontSize: 13, bold: true, color: REPORT_COLORS.forest, margin: [0, 8, 0, 10] } },
    watermark: { text: 'UJIMORA', color: REPORT_COLORS.sage, opacity: 0.065, bold: true, angle: -30 },
    header: {
      margin: [36, 20, 36, 0],
      stack: [
        { columns: [{ svg: REPORT_LOGO, width: 40 }, { text: 'Ujimora', bold: true, fontSize: 20, color: REPORT_COLORS.forest, margin: [6, 3, 0, 0] }, { text: 'ADMINISTRATIVE REPORT', alignment: 'right', fontSize: 8, color: REPORT_COLORS.forest, margin: [0, 10, 0, 0] }] },
...
export async function pdfBlob(report: ExportReport): Promise<Blob> {
  ...
  pdfMake.addFonts({ Outfit: {
    normal: new URL('/fonts/export/Outfit-Regular.ttf', window.location.origin).href,
    bold: new URL('/fonts/export/Outfit-Bold.ttf', window.location.origin).href,
    italics: new URL('/fonts/export/Outfit-Regular.ttf', window.location.origin).href,
    bolditalics: new URL('/fonts/export/Outfit-Bold.ttf', window.location.origin).href,
  } })
