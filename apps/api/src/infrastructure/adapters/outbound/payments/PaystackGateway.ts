...
export class PaystackGateway implements PaymentGatewayPort {
  ...
  async initializeTransaction(intent: DonationIntentEntity): Promise<PaymentGatewayInitResult> {
    ...
    const json = await this.request<PaystackInitData>('POST', '/transaction/initialize', body)
    if (!json.status || !json.data) {
      throw new AppError(`Paystack initialization failed: ${json.message ?? 'unknown error'}`, 502)
    }
  ...
  async initializeCharge(params: InitializeChargeParams): Promise<PaymentGatewayInitResult> {
    if (!this.isConfigured()) {
      throw new AppError('Payments are not configured', 501)
    }
  ...
  async refundPayment(
    ...
    if (!json.status || !json.data) {
      throw new AppError(`Paystack refund failed: ${json.message ?? 'unknown error'}`, 502)
    }
    return {
  ...
  async listBanks(currency: string, type?: string): Promise<PaymentGatewayBank[]> {
    if (!this.isConfigured()) {
      throw new AppError('Payments are not configured', 501)
    }
    ...
    const json = await this.request<PaystackBankData[]>('GET', `/bank?${params.toString()}`)
    if (!json.status || !json.data) {
      throw new AppError(`Paystack bank listing failed: ${json.message ?? 'unknown error'}`, 502)
    }
  ...
  async resolveAccount(accountNumber: string, bankCode: string): Promise<{ accountName: string }> {
    ...
    if (!json.status || !json.data?.account_name?.trim())
      throw new AppError(
        'Account name could not be verified. Submit for payout review or choose another account.',
        422,
      )
    return { accountName: json.data.account_name.trim() }
  }

  async createTransferRecipient(params: CreateTransferRecipientParams): Promise<string> {
    if (!this.isConfigured()) {
      throw new AppError('Payments are not configured', 501)
    }
    const body = {
    ...
    const json = await this.request<PaystackRecipientData>('POST', '/transferrecipient', body)
  ...
  async initiateTransfer(params: InitiateTransferParams): Promise<PaymentGatewayTransferResult> {
    if (!this.isConfigured()) {
      throw new AppError('Payments are not configured', 501)
    }
    ...
    if (!json.status || !json.data) {
      throw new AppError(`Paystack transfer failed: ${json.message ?? 'unknown error'}`, 502)
    }
    return {
  ...
  async finalizeTransfer(code: string, otp: string): Promise<void> {
    const result = await this.request('POST', '/transfer/finalize_transfer', {
      transfer_code: code,
      otp,
    })
    if (!result.status)
      throw new AppError(
        'Paystack could not authorize this transfer. Refresh its status before retrying.',
        422,
      )
  }
  async resendTransferOtp(code: string): Promise<void> {
    const result = await this.request('POST', '/transfer/resend_otp', {
      transfer_code: code,
      reason: 'resend_otp',
    })
    if (!result.status)
      throw new AppError('Paystack could not resend the OTP. Refresh the transfer status.', 422)
  }
  async verifyTransfer(reference: string): Promise<PaymentGatewayTransferResult> {
    if (!this.isConfigured()) {
      throw new AppError('Payments are not configured', 501)
    }
  ...
  async getBalance(): Promise<PaymentGatewayBalance[]> {
    ...
    if (!json.status || !json.data) {
      throw new AppError(`Paystack balance lookup failed: ${json.message ?? 'unknown error'}`, 502)
    }
    return json.data.map((b) => ({
      currency: b.currency,
