...
it('mounts the real AI routes with session and administrator checks', async () => {
  ...
  const signup = await request(app).post('/api/v1/auth/register').send({ legalAcceptance: { version: '2026-09-12', acceptedTerms: true, ageConfirmed: true }, email, password, name: 'AI Test' }).expect(201)
  ...
  await request(app).post('/api/v1/ai-writing').set('Authorization', `Bearer ${token}`).send({ consentToExternalProcessing: true, text: 'An honest campaign story.', action: AiWritingAction.IMPROVE_CLARITY }).expect(503)
