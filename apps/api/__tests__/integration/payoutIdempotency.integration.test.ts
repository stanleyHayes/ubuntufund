  ...
  it('reconciliation repairs a PAID-but-unsettled payout (crash between transition and effect)', async () => {
    const fetchMock = vi.fn(async (url: unknown, opts: unknown) => {
      ...
      if (u.includes('/transaction/initialize')) {
        ...
        return json({
          status: true,
          data: { authorization_url: `x/${reference}`, access_code: 'a', reference },
        })
      }
      if (u.includes('/bank/resolve')) return json({ status: true, data: { account_name: 'Jane' } })
    ...
    const camp = await request(app)
      ...
      .send({
        title: 'G5 Repair Campaign',
        description: 'A campaign to test settlement repair',
        goalAmount: 5000,
        currency: 'GHS',
    ...
    const checkout = await request(app)
      ...
      .expect(201)
    const rawCharge = JSON.stringify({
      event: 'charge.success',
      data: {
        reference: checkout.body.data.reference,
        amount: 100000,
        fees: 0,
        currency: 'GHS',
        status: 'success',
    ...
    await request(app)
      ...
      .set('x-paystack-signature', sign(rawCharge))
    ...
    await request(app)
      ...
      .expect(201)
    await CampaignModel.findByIdAndUpdate(campaignId, { status: 'funded', raisedAmount: 5000 })
    ...
    await request(app)
      ...
      .send({ reviewNote: 'Ownership and receiving capacity verified for test recipient.' })
      .expect(200)
