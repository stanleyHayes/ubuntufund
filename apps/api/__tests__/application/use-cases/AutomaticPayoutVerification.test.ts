...
it('rechecks after the balance lookup and refuses to reserve or send after revocation', async () => {
  ...
  const repo = { findById: async () => ({ ...payout, toPlain: () => payout }), transitionToProcessing: vi.fn() }
  ...
  expect(guard.run).toHaveBeenCalledWith('owner', expect.any(Function), { ...payout, recipientCode: 'RCP_fixture' })
