...
describe('cashout options', () => {
  ...
  const recipients = { findLatestByCampaignId: vi.fn(async () => ({ accountName: 'Owner', accountNumber: '0241234567', type: 'mobile_money', toPlain: () => ({}) })) };
  ...
  it('returns net eligible proceeds and only the account suffix', async () => {
    expect(await uc.execute('campaign', { userId: 'owner' })).toEqual({ requiresEarlyCashout: false, eligible: 100, currency: 'GHS', fees: { earlyMaxWithdrawalPercent: 80 }, recipient: { accountName: 'Owner', last4: '4567', type: 'mobile_money', verificationStatus: 'needs_review', resolvedAccountName: undefined } });
  });
});
