...
describe('cashout options', () => {
  ...
  const uc = new GetCampaignPayoutOptionsUseCase({ findById: async () => ({ creatorId: 'owner' }) } as never, balances as never, recipients as never, { resolvePayoutsConfig: async () => ({ earlyMaxWithdrawalPercent: 80 }) } as never);
  ...
  it('returns net eligible proceeds and only the account suffix', async () => {
    expect(await uc.execute('campaign', { userId: 'owner' })).toEqual({ eligible: 100, currency: 'GHS', fees: { earlyMaxWithdrawalPercent: 80 }, recipient: { accountName: 'Owner', last4: '4567', type: 'mobile_money' } });
  });
});
