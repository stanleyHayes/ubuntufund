...
async function main() {
  ...
  const report = await auditAccountClosureIntegrity(mongoose.connection, { before: new Date(before), after: after || undefined, limit: Number(limit) });
  process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
  if (report?.findings.length) process.exitCode = 2;
}
main().catch(() => { process.stderr.write('Account closure audit failed. Check arguments, database access and snapshot support.\n'); process.exitCode = 1; })
