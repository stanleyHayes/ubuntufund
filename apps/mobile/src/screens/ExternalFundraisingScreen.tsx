...
export default function ExternalFundraisingScreen() {
  ...
  async function open() {
    if (!campaign?.slug || campaign.status !== 'active') return
  ...
  return <ScrollView style={{ flex: 1, backgroundColor: palette.background }} contentContainerStyle={{ padding: 24, gap: 20 }}>
    ...
    {campaign?.status === 'active' && campaign.slug ? <Button mode="contained" icon="open-in-new" loading={opening} disabled={opening || !!loadError} onPress={() => void open()}>Continue in browser</Button> : <Text>This campaign is not accepting donations right now.</Text>}
