...
export async function readBiometricCredential(): Promise<{ userId: string; refreshToken: string }> {
  if (!(await biometricCapability()).available) throw new Error('Biometrics are unavailable or changed. Sign in with your password instead.')
  const value = await SecureStore.getItemAsync(VAULT_KEY, OPTIONS)
  if (!value) throw new Error('Biometric access changed or expired. Sign in with your password instead.')
  const parsed = JSON.parse(value) as { userId?: unknown; refreshToken?: unknown }
  if (typeof parsed.userId !== 'string' || typeof parsed.refreshToken !== 'string') throw new Error('Saved sign-in is unavailable. Sign in with your password instead.')
