...
test('wallet donation always requires visible terms acceptance, with an optional message', async ({ page }) => {
  ...
  await page.route('**/api/v1/campaigns/example/donate', route => route.fulfill({ json: { data: {} } }))
  await page.goto('/campaigns/example')
  ...
  const confirm = dialog.getByRole('button', { name: 'Confirm Donation' })
  const acceptance = dialog.getByRole('checkbox', { name: /I am at least 18/ })
  await expect(acceptance).toBeVisible()
  await expect(confirm).toBeDisabled()
  await acceptance.check()
  await expect(confirm).toBeEnabled()
  await acceptance.uncheck()
  await dialog.getByLabel('Message (optional)').fill('Thank you for helping our community')
  await expect(acceptance).not.toBeChecked()
  await expect(confirm).toBeDisabled()
  await expect(dialog.getByRole('link', { name: 'Terms of Use' })).toHaveAttribute('href', '/terms')
  await acceptance.check()
  await expect(confirm).toBeEnabled()
  await acceptance.uncheck()
  await expect(confirm).toBeDisabled()
  await acceptance.check()
  const request = page.waitForRequest(request => request.method() === 'POST' && request.url().endsWith('/campaigns/example/donate'))
  await confirm.click()
  expect((await request).postDataJSON()).toMatchObject({ paymentMethod: 'wallet', donorName: 'Test', isAnonymous: false, message: 'Thank you for helping our community', legalAcceptance: { version: '2026-09-12', acceptedTerms: true, ageConfirmed: true } })
  await expect(dialog).toHaveCount(0)
  await page.getByRole('button', { name: 'Donate with wallet' }).click()
  await expect(dialog.getByRole('checkbox', { name: /I am at least 18/ })).toBeVisible()
  await expect(dialog.getByRole('checkbox', { name: /I am at least 18/ })).not.toBeChecked()
  await dialog.getByRole('spinbutton', { name: 'Amount' }).fill('10')
  await dialog.getByRole('button', { name: 'Wallet', exact: true }).click()
  await expect(confirm).toBeDisabled()
  await dialog.getByRole('checkbox', { name: /I am at least 18/ }).check()
  await dialog.getByRole('checkbox', { name: 'Donate anonymously', exact: true }).check()
  const blankRequest = page.waitForRequest(request => request.method() === 'POST' && request.url().endsWith('/campaigns/example/donate'))
  await confirm.click()
  const blankPayload = (await blankRequest).postDataJSON()
  expect(blankPayload.message).toBeUndefined()
  expect(blankPayload.donorName).toBeUndefined()
  expect(blankPayload.isAnonymous).toBe(true)
