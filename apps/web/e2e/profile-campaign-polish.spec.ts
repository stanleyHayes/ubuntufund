...
test('wallet donation always requires visible terms acceptance, with an optional message', async ({ page }) => {
  ...
  await confirm.click()
  expect((await request).postDataJSON()).toMatchObject({ paymentMethod: 'wallet', message: 'Thank you for helping our community', legalAcceptance: { version: '2026-09-12', acceptedTerms: true, ageConfirmed: true } })
  await expect(dialog).toHaveCount(0)
  await page.getByRole('button', { name: 'Donate with wallet' }).click()
