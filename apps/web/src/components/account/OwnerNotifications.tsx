...
export function OwnerNotifications() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('ujimora:notifications-changed', visibleRefresh)
    return () => {
      clearInterval(timer)
      window.removeEventListener('focus', visibleRefresh)
      window.removeEventListener('ujimora:notifications-changed', visibleRefresh)
    }
  }, [refresh])
      ...
      {items.slice(0, 10).map((item) => (
        <Box key={item.id} sx={{ py: 2, borderBottom: '1px solid', borderColor: 'divider' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            {!item.read && <Chip label="New" size="small" />}
          </Box>
