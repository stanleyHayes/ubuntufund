...
export function CampaignCreationExtras({ options, invitations, setInvitations, split, setSplit, rows, setRows }: {
  ...
  return <Box sx={{ mt: 3, borderTop: '1px solid', borderColor: 'divider', pt: 3 }}>
    ...
    <Typography variant="body2" color="text.secondary">{!options.splitEnabled ? 'Split proceeds are not currently enabled.' : !options.canSplit ? 'Split proceeds require a plan that includes campaign collaboration and split proceeds.' : 'Create a draft allocation. Every beneficiary must consent before a split can be activated; this does not transfer money.'}</Typography>
