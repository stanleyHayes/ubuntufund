...
export function CampaignForm() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (Object.keys(all).length === 0) {
      // Preserve the existing create behaviour — mock success, no data saved.
      setSubmitted(true)
    } else {
    ...
    return (
      <Box sx={{ textAlign: 'center', py: { xs: 2, sm: 4 } }}>
        ...
        <Typography sx={{ mt: 1.5, color: INK_SECONDARY, maxWidth: 440, mx: 'auto', lineHeight: 1.6 }}>
          Campaign created successfully! (This is a mock -- no data was saved.) Once live, every cedi raised is tracked and every
          donor is thanked.
        </Typography>
      </Box>
    )
    ...
    <Box component="form" onSubmit={handleSubmit} noValidate>
      ...
      >
        ...
        {step < STEPS.length - 1 ? (
          ...
          <Button type="submit" variant="contained" color="secondary" size="large" startIcon={<CheckRoundedIcon />}>
            Publish campaign
          </Button>
        )}
      </Box>
