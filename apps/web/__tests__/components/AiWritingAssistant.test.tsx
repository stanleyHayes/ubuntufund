...
describe('Writing assistant', () => {
  it('reports the original generated suggestion from inside the assistant', async () => {
    vi.mocked(api.post).mockResolvedValueOnce({ requestId: 'request-id', result: 'Generated suggestion to report.', remainingRequests: 19 }).mockResolvedValueOnce({ id: 'report', status: 'pending' })
    render(view('Original'))
    fireEvent.click(await screen.findByRole('checkbox', { name: /I agree to send/ }))
    ...
    fireEvent.change(screen.getByRole('textbox', { name: 'What happened?' }), { target: { value: 'Please review this generated content.' } })
    ...
    expect(api.post).toHaveBeenLastCalledWith('/safety/reports', { targetType: 'ai_output', targetId: 'request-id', generatedText: 'Generated suggestion to report.', reason: 'harassment', description: 'Please review this generated content.' })
  })
  it('requires separate permission for each transmission', async () => {
    vi.mocked(api.post).mockResolvedValue({ result: 'Suggestion', remainingRequests: 19 })
    render(view('Original'))
    const button = await screen.findByRole('button', { name: 'Get suggestion' })
    expect(button).toBeDisabled()
    expect(api.post).not.toHaveBeenCalled()
    fireEvent.click(screen.getByRole('checkbox', { name: /I agree to send/ }))
    fireEvent.click(button)
    await screen.findByText('Suggestion')
    expect(api.post).toHaveBeenCalledWith('/ai-writing', expect.objectContaining({ consentToExternalProcessing: true }))
    expect(button).toBeDisabled()
  })
