...
5. Check the transfer approval configuration. If Paystack returns `otp` for a single (non-batched) payout, the payout stays PROCESSING and Admin → Payouts shows **Authorize existing transfer** (enter the six-digit Paystack business OTP; the recipient never enters it), **Resend OTP** and **Check Paystack status**. These act on the existing transfer code and never create a new transfer. Ledger settlement follows Paystack verification, not the authorization response. Batched (multi-leg) payouts have no OTP controls in Ujimora: authorize those legs in the Paystack dashboard, then reconcile. An admin approval in Ujimora does not by itself satisfy Paystack OTP. Do not disable OTP casually; see `docs/payments/automatic-payouts.md` for the Confirm-transfers preference and the optional transfer-approval URL.
