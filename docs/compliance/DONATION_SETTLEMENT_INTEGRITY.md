...
## Creator withdrawal currency consistency — 2026-09-13

Bank/ mobile-money creator withdrawals previously took currency from a pre-transaction balance lookup without comparing the balance actually reserved. A currency change before reservation could therefore send the stale currency. The use case now compares the returned reserved balance inside the transaction and throws409 on mismatch, rolling back the reservation before payout creation/provider transfer. The wallet rail already filters its reservation by GHS.

The new real-database regression changes currency between the initial lookup and transaction. Before the fix it received201; after the fix it verifies409, the original available amount, zero payout records and no transfer call. All31 creator-withdrawal integration tests pass (19.96seconds), API types and affected lint pass. Evidence: `/tmp/ujimora-creator-currency-before.log`, `/tmp/ujimora-creator-currency-tests.log`, `/tmp/ujimora-creator-currency-types.log`, `/tmp/ujimora-creator-currency-lint.log`. Full1,210-test baseline above predates this delta. Historical reconciliation and actual provider currency acceptance remain open; no production funds were moved.
