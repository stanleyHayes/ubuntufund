...
The requester can refresh the paginated history and read a response in Settings. Web can download the request and response as JSON; native can share that same information through the system share sheet. This artifact contains the request and the reviewer-provided response. It is **not** an automatic export of every collection. Users without account access can contact `legal@ujimora.com`; the shared privacy notice also identifies the Ghana Data Protection Commission. When staff publish a response to the account, the requester is emailed through the encrypted account-mail queue that a response is ready in Settings. The email carries the request reference and a Settings link, never the response itself. Verified external delivery sends no account email. Nothing is sent when account email delivery is not configured, so the UI still tells the requester to check the response here.

## Operator workflow

13 September authorization follow-up: progress/response publication now conditionally writes the reviewing administrator's current role, credential version and nonclosed account inside the same transaction as the request revision and audit event. A role removal, credential rotation or closure after HTTP authentication denies the mutation. Eight data-rights integration tests pass, including all three interleavings and existing request isolation, audit rollback, duplicate/stale review and external-delivery controls. API types/lint pass; logs `/tmp/ujimora-rights-authorization-{tests,types,lint}.log`, sessions 61554/60923/62875 terminal exit 0. This controls who may review; it does not prove the completeness of the supplied data or fulfil processor access obligations.
...
Cleanup/review concurrency follow-up: the worker's conditional pending → review_required transition now increments the same revision used by staff saves. A form read before cleanup can no longer overwrite a review without first refreshing the changed evidence state. The worker does not replace review notes or nextReviewAt. The real-database retry test verifies both preserved fields, one revision advance and no further advance on a repeated sweep. All four erasure tests and API types pass; `/tmp/ujimora-cleanup-review-revision-{tests,types}.log`.
...
### Open-account predicate consistency — 13 September 2026

Further testing found explicit-null accounts were rejected by the user repository at authentication (401), before the missing-only data-rights predicates could run. User lookup by ID/email, guarded account save/closure, request submission and account response publication now consistently treat missing/null deletion dates as open; timestamped closure stays denied. This also clarifies the preceding tombstone finding: ordinary authenticated access already rejected explicit-null records, although other current-account transaction fences accepted them. Persisting an actual closure timestamp remains necessary for consistent enforcement across those paths.

The new regression starts from an explicit-null account and verifies authenticated request creation and readable staff response. Before the change it failed401; after the change all15 data-rights/erasure tests and7 authentication/deleted-account tests pass. API types/lint pass. Evidence: `/tmp/ujimora-rights-null-before.log`, `/tmp/ujimora-rights-null-final.log`, `/tmp/ujimora-null-auth-tests.log`, `/tmp/ujimora-rights-null-types.log`, `/tmp/ujimora-rights-null-lint.log`. No production account was modified. Historic completed deletion requests lacking tombstones still require a reviewed inventory/repair; this change does not claim that reconciliation occurred.

Historical closure review now has a tested read-only inventory command; see [ACCOUNT_CLOSURE_AUDIT.md](ACCOUNT_CLOSURE_AUDIT.md). It checks structural request/account markers, not actual processor erasure or all retained data. Production inventory and repairs remain open.
