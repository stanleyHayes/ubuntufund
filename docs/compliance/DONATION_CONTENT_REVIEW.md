...
## Wallet attribution correction — 13 September 2026

The wallet modal previously sent non-anonymous donations without a donor-name field, and the legacy wallet use case did not forward one. The form now displays an editable public name (initially the signed-in account name), an explicit anonymous choice, and clear pending-review expectations. Name-required validation applies when public attribution is selected; anonymous submissions omit names. The route validates the optional bounded name and the use case forwards the trimmed choice with the existing legal acceptance. Review gates remain unchanged and historical missing names are not replaced with current profile names.

Browser verification checks named and anonymous payloads alongside always-visible unchecked terms acceptance. Ten focused API tests, API/web types and affected lint pass (`/tmp/ujimora-wallet-name-{browser,api,web-types,api-types,lint}.log`). This is prospective attribution capture; existing pending/unreviewed donations remain anonymous until eligible reviewed attribution exists. No production donation was altered.
