...
13 September browser refresh: guest message and both name-only anonymity consent flows pass against current checkout. The initial3 failures came from mocked campaigns without endDate; fixtures now include a future deadline, matching the real campaign contract. The accompanying7 legal/account agreement/privacy-download flows passed without changes. Evidence: `/tmp/ujimora-browser-compliance-1fa85fb.log`, `/tmp/ujimora-browser-consent-final.log`. No live payment, public post or privacy response was sent.
