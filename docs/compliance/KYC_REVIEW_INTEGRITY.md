...
## Money-out consumers, round 2 — 2026-09-25

Beneficiary (split-proceeds) payouts now pass the same campaign gate as campaign payouts: a blocked, draft, deleted or disputed campaign refuses both the request (before any pending → available clearing) and the approval, which write-fences the campaign inside its transaction. Only the beneficiary or the campaign owner can register a beneficiary destination or request a beneficiary payout; staff cannot. Each request is bound to a fingerprint of the destination it was made against, the destination cannot be replaced while a request is pending, and approval refuses a changed destination, a request made before binding, a destination the approver entered, and one entered by anyone other than the beneficiary or owner. Pending beneficiary requests can be rejected (admin, with a reason) or cancelled (beneficiary or owner), returning exactly what they cleared.

Affiliate commission payouts now require a verified email and current identity verification at request and again inside the approval transaction. The money-out gate reports an unverified email as such ("Verify your email address…"), never as an identity problem, on every rail.

Open decision (owner/compliance): `name_matched` compares the name a person typed with the provider-held name, not with their verified legal name, so a KYC-verified creator can still withdraw to an account held by someone else without staff review. This is not blocked. As a detective control, a creator bank/MoMo withdrawal to an account whose provider-held name does not match the creator's current verified legal name now writes an audit entry (`creator_withdrawal.destination_not_legal_name`, warning) for after-the-fact review. Whether to require first-party destinations (refuse or route such withdrawals to staff review) is a policy decision still to be made.
