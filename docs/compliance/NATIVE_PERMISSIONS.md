...
### Current APK16KB emulator startup
...
## RELRO alignment fixed for all 64-bit libraries — 25 September 2026

A 12-agent research pass checked every owning artifact on Maven Central, Google Maven and the React Native Maven repository. Independent verifiers re-downloaded the artifacts. No published version fixes all failing libraries:
- React Native/JSI/Hermes tooling fail in 0.83.10 through 0.88.0-rc.2.
- fbjni 0.7.0 is the latest release.
- Fresco 3.6.0 through 3.8.0 all fail.
- graphics-path fails in every release (only an AndroidX snapshot passes).
- WebRTC SDK 144.x and the LiveKit 3.0 artifact fail.
- expo-image hard-codes decoder 3.0.5 in every SDK.

NDK r30's libc++ passes, but the AARs carry their own copies. Upstream links with max-page-size=16384 but not common-page-size=16384.

Fix: `apps/mobile/plugins/relro16k.gradle` (applied by `withPrebuiltRelroAlignment`) runs after `strip*DebugSymbols`. For a misaligned 64-bit ET_DYN library it:
- raises PT_GNU_RELRO p_memsz to the next 16 KB boundary;
- grows the containing writable PT_LOAD p_memsz to the same end (zero-fill);
- updates `.relro_padding` sh_size.

It refuses when writable data follows RELRO in the same segment, or when another segment overlaps the rounding window. It skips 32-bit and already-aligned files, so repeated runs are no-ops. The AOSP bionic linker (main = android16, android15) protects [page_start(vaddr), page_end(end)), so on 16 KB devices the protected range is unchanged. The 16 KB app-compat mode does not apply (all 16 KB LOAD aligned). None of the 27 libraries had writable data in the rounding window. A reviewer re-derived this and ran patched libraries on 16 KB and 4 KB emulators.

Evidence from the release build (Java 17, production API URL, debug signing):
- Hook log: patched 27, already aligned 21, refused 0.
- `inspect-android-native.py`: APK 48/48 pass. The App Bundle's 48 64-bit libraries are byte-identical to the APK's.
- All 27 patched libraries are byte-identical to the independently reviewed Python reference patcher output.
- `zipalign -c -P 16 4` passes. bundletool 1.18.1 `validate` passes, with `PAGE_ALIGNMENT_16K`.
- Split APKs generated by bundletool from the AAB install on emulator-5580 (PAGE_SIZE 16384, `bionic.linker.16kb.app_compat.enabled=false`) and cold-start.

The emulator was restarted with `-dns-server 8.8.8.8,1.1.1.1` after its DNS forwarder stopped resolving. Manual UI pass against the production API, read-only and signed out: home with live campaign images, campaign detail, Android donate form (crypto hidden, amount entry), Explore, Profile and login. No fatal or React host errors.

Bugs found during this pass and fixed:
- `BrandedTextInput` passed `outlineStyle` to flat Paper inputs. Paper forwards it to the native TextInput, whose React Native 0.83 `outlineStyle` prop rejects objects. On Android that tore down the React host, and every form screen (donate, KYC, campaign create, wallet…) rendered blank.
- The home tab and the web public campaign page treated only `active` campaigns as open, hiding funded campaigns that still accept donations.
