...
Missing-reference records now participate in bounded, rotating stale scans instead of being silently excluded. Missing references and absent original providers produce `blocked` counts and bounded issue rows containing only deposit ID, provider and reason. The admin crypto panel displays investigation guidance and exports these rows through the existing branded report controls. It offers no replacement-deposit creation, reference editing or manual settlement override. Provider restoration or authoritative recovery procedures still need operator evidence; visibility does not resolve the underlying obligation.

Current verification: 12 API crypto integration tests plus one admin display/export test pass, with API/admin types/lint successful. Real Mongo fixtures confirm absent/empty references and unavailable providers surface without changing funds, status or financial timestamps; the admin test verifies reasons and explicit export fields exclude donor email. Logs `/tmp/ujimora-crypto-issues-{tests,admin,checks}.log`; sessions 19953/72398/29485 terminal exit 0. Live provider recovery and a rendered browser check remain unverified.
